...
class SegmanConsoleSummaryHook(Hook):
    ...
    def _print_val_block(self, runner):
        ...
        # 对齐跨项目备忘（如 BEVANet ``1.md`` §4）：终端表头易把「mIoU」与前景 IoU 混淆
        miou = buf.output.get('val/mIoU', buf.output.get('mIoU'))
        legend_parts = []
        if miou is not None:
            try:
                legend_parts.append(
                    f'mIoU(mean)={float(miou):.4f}（两类算术平均）')
            except (TypeError, ValueError):
                legend_parts.append(f'mIoU(mean)={miou}')
        if iou_fg is not None:
            try:
                legend_parts.append(
                    f'IoU_fg={float(iou_fg):.4f}（与 save_best / 早停 monitor 一致）')
            except (TypeError, ValueError):
                legend_parts.append(f'IoU_fg={iou_fg}')
        if legend_parts:
            print(
                self._c('[口径] ', 'cyan') + ' | '.join(legend_parts),
                flush=True)
        print(flush=True)
