...
#
# - ``segman_binary_ckpt_root='data/checkpoints2'``：与 TransNeXt ``work_dir=data/checkpoints2`` 同层级命名
# - 主目录：``<仓库根>/data/checkpoints2/train_<时间>/``（``save_best='val/IoU'`` 存 IoU 最优）
# - ``<仓库根>/data/checkpoints2/val_loss_best/<run_ts>/``：按更小 val loss 另存权重
...
segman_binary_ckpt_root = 'data/checkpoints2'
