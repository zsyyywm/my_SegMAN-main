...
#
# 指标口径（与仓库根 ``1.md`` / BEVANet §4 类备忘一致）：日志里的 **mIoU** 为两类 IoU 的算术平均；
# **save_best / 早停** 以 ``val/IoU``（前景 IoU）为准，勿与 mIoU(mean) 混为一谈。
#
...
# - 产出目录：``<仓库根>/data/checkpoints1/train_<时间>/``（与 BEVANet ``data/checkpoints1`` 对齐）
#
# 在 ``segmentation`` 目录执行:
...
# 预训练权重：ADE20K 上训好的 **SegMAN-T 整网**（下载后放 ``segmentation/pretrained/`` 并命名为下述文件名）。
# 官方 Google Drive（根目录 README.md ADE20K 表「SegMAN-T」/ SETUP.md §6）:
