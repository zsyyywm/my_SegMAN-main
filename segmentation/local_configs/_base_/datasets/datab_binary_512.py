# DataB 二分类。请在 ``segmentation/`` 下启动训练；默认 ``../../../DataA-B/DataB``。
# 其它路径：``export SEG_DATAB_ROOT=/绝对路径/DataB``。
# 数据增强与验证 pipeline 与 ``dataa_binary_512.py``、sci/TransNeXt DataA 配置对齐。
import os
...
img_norm_cfg = dict(
    ...
    to_rgb=True)
crop_size = (512, 512)
_DATAB_SHORT_EDGES = [int(512 * x * 0.1) for x in range(5, 21)]
